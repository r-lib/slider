...
[`slide()`](https://slider.r-lib.org/reference/slide.md) that iterate
over multiple inputs in parallel. They are parallel in the sense that
each input is processed in parallel with the others, not in the sense of
multicore computing. These functions work similarly to `map2()` and
`pmap()` from purrr.
