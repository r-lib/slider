...
test_that("hop_index_int() works", {
  expect_identical(hop_index_int(1L, 1, 1, 1, ~.x), 1L)
})

test_that("hop_index_int() can coerce", {
  expect_identical(hop_index_int(1, 1, 1, 1, ~.x), 1L)
})

test_that("hop_index_dbl() works", {
  expect_identical(hop_index_dbl(1, 1, 1, 1, ~.x), 1)
})

test_that("hop_index_dbl() can coerce", {
  expect_identical(hop_index_dbl(1L, 1, 1, 1, ~.x), 1)
})

test_that("hop_index_chr() works", {
  expect_identical(hop_index_chr("x", 1, 1, 1, ~.x), "x")
})

test_that("hop_index_chr() can coerce", {
  expect_identical(hop_index_chr(1, 1, 1, 1, ~.x), "1")
})

test_that("hop_index_lgl() works", {
  expect_identical(hop_index_lgl(TRUE, 1, 1, 1, ~.x), TRUE)
})

test_that("hop_index_lgl() can coerce", {
  expect_identical(hop_index_lgl(1, 1, 1, 1, ~.x), TRUE)
})

test_that("hop_index_raw() works", {
  expect_identical(hop_index_raw(raw(1), 1, 1, 1, ~.x), raw(1))
})

# ------------------------------------------------------------------------------
# data frame suffix tests

test_that("hop_index_dfr() works", {
  expect_equal(
    hop_index_dfr(1:2, 1:2, c(1, 1), c(1, 2), ~.x),
    data.frame(...1 = c(1, 1), ...2 = c(NA, 2))
  )

  x <- 1:2
  expect_equal(
    hop_index_dfr(x, 1:2, c(1, 1), c(1, 2), ~data.frame(x = .x)),
    data.frame(x = c(1, 1, 2))
  )
})

test_that("hop_index_dfc() works", {
  expect_equal(
    hop_index_dfc(1:2, 1:2, c(1, 1), c(1, 2), ~.x),
    data.frame(...1 = c(1, 1), ...2 = c(1, 2))
  )

  x <- 1:2
  expect_equal(
    hop_index_dfc(x, 1:2, c(1, 1), c(1, 2), ~data.frame(x = .x)),
