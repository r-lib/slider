...
test_that("empty `.x`, but size `n > 0` `.starts` and `.stops`: sizes and types are checked first", {
  ...
  expect_error(hop(integer(), 1, "x", ~.x), class = "vctrs_error_index_bad_type")
})

test_that(".starts must not contain NA values", {
  expect_error(hop(1:2, c(1, NA), 1:2, identity), "found at location[(]s[)]: 2")
  expect_error(hop(1:2, c(NA, 1), 1:2, identity), "found at location[(]s[)]: 1")
})

test_that(".stops must not contain NA values", {
  expect_error(hop(1:2, 1:2, c(1, NA), identity), "found at location[(]s[)]: 2")
  expect_error(hop(1:2, 1:2, c(NA, 1), identity), "found at location[(]s[)]: 1")
})
...
test_that("`.starts` and `.stops` must be integerish", {
  expect_error(hop(1, "x", 1, identity), class = "vctrs_error_index_bad_type")
  expect_error(hop(1, 1, "x", identity), class = "vctrs_error_index_bad_type")
})
