...
test_that("pslide_*() retains names of first input", {
        expect_identical(
                pslide_vec(list(c(x = 1L), c(y = 1L)), ~ .x + .y),
                c(x = 2L)
        )
        expect_identical(
                pslide_int(list(c(x = 1L), c(y = 1L)), ~ .x + .y),
                c(x = 2L)
        )
})
...
test_that(".step produces typed `NA` values", {
        expect_identical(
                pslide_int(list(1:3, 1:3), ~.x, .step = 2),
                c(1L, NA, 3L)
        )
        expect_identical(
                pslide_dbl(list(1:3, 1:3), ~.x, .step = 2),
                c(1, NA, 3)
        )
        expect_identical(
        ...
        expect_identical(
                pslide_vec(list(1:3, 1:3), ~.x, .step = 2),
                c(1L, NA, 3L)
        )
        expect_identical(
