...
test_that("can use millisecond Durations with POSIXct", {
        ...
        expect_equal(
                slide_index(
                        ...
                        i,
                        identity,
                        .before = lubridate::dmilliseconds(1)
                ),
                list(
                        1L,
                        1:2,
        ...
        expect_snapshot({
                (expect_error(
                        slide_index(
                                ...
                                1:2,
                                identity,
                                .before = ~ rep(NA_integer_, length(.x))
                        )
                ))
                (expect_error(
                        slide_index(
                                ...
                                1:2,
                                identity,
                                .after = ~ rep(NA_integer_, length(.x))
                        )
                ))
        })
})
...
test_that(".before/.after - function must have 1 argument", {
        # Base R errors
        expect_error(slide_index(
                1,
                1,
                identity,
                .before = function(x, y) x + y
        ))
