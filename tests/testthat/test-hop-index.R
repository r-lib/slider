...
test_that(".i must be ascending", {
        expect_snapshot({
                (expect_error(
                        hop_index(1:2, 2:1, 1:2, 1:2, identity),
...
test_that(".starts must be ascending", {
        expect_snapshot({
                (expect_error(
                        hop_index(1:2, 1:2, 2:1, 1:2, identity),
...
test_that(".stops must be ascending", {
        expect_snapshot({
                (expect_error(
                        hop_index(1:2, 1:2, 1:2, 2:1, identity),
                        class = "slider_error_endpoints_must_be_ascending"
                ))
        })
})
...
test_that("empty input returns a list, but after the index size check", {
        expect_equal(
                hop_index(integer(), integer(), integer(), integer(), ~.x),
                list()
        )
        expect_error(
                hop_index(integer(), 1, integer(), integer(), ~.x),
                class = "slider_error_index_incompatible_size"
        )
})
...
test_that("empty `.x` and `.i`, but size `n > 0` `.starts` and `.stops`: sizes and types are checked first", {
        expect_snapshot({
                (expect_error(
                        hop_index(integer(), integer(), 1:3, 1:2, ~.x),
                        class = "vctrs_error_incompatible_size"
                ))
                (expect_error(
                        hop_index(integer(), integer(), 1, "x", ~.x),
...
test_that(".i must not contain NA values", {
        expect_snapshot({
                (expect_error(
                        hop_index(1:2, c(1, NA), 1:2, 1:2, identity),
                        class = "slider_error_index_cannot_be_na"
                ))
                (expect_error(
                        hop_index(1:2, c(NA, 1), 1:2, 1:2, identity),
                        class = "slider_error_index_cannot_be_na"
                ))
        })
})
...
test_that(".starts must not contain NA values", {
        expect_snapshot({
                (expect_error(
                        hop_index(1:2, 1:2, c(1, NA), 1:2, identity),
                        class = "slider_error_endpoints_cannot_be_na"
                ))
                (expect_error(
                        hop_index(1:2, 1:2, c(NA, 1), 1:2, identity),
                        class = "slider_error_endpoints_cannot_be_na"
                ))
        })
})
...
test_that(".stops must not contain NA values", {
        expect_snapshot({
                (expect_error(
                        hop_index(1:2, 1:2, 1:2, c(1, NA), identity),
                        class = "slider_error_endpoints_cannot_be_na"
                ))
                (expect_error(
                        hop_index(1:2, 1:2, 1:2, c(NA, 1), identity),
                        class = "slider_error_endpoints_cannot_be_na"
                ))
        })
})
...
test_that("can use `%m-%` and `add_with_rollback()` to solve month rollback issues", {
        ...
        starts <- lubridate::add_with_rollback(
                i,
                -months(1),
                roll_to_first = TRUE
        )
