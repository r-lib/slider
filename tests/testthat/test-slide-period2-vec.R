...
test_that("`slide_period2_vec()` falls back to `c()` method as required", {
  ...
  expect_identical(slide_period2_vec(1:3, 1:3, new_date(1:3), "day", ~foobar(.x), .ptype = foobar(integer())), foobar(1:3))
  expect_condition(slide_period2_vec(1:3, 1:3, new_date(1:3), "day", ~foobar(.x), .ptype = foobar(integer())), class = "slider_c_foobar")
