...
test_that("empty input works with `.complete = TRUE` (#111)", {
  expect_equal(
    slide_period_dbl(integer(), new_date(), "year", ~.x, .complete = TRUE),
    double()
  )
})
...
test_that("`.ptype = NULL` returns `NULL` with size 0 `.x`", {
  expect_equal(
    slide_period_vec(integer(), new_date(), "day", ~.x, .ptype = NULL),
    NULL
  )
})
...
test_that("slide_period_chr() cannot coerce", {
  expect_error(
    slide_period_chr(1, new_date(0), "day", ~.x),
    class = "vctrs_error_incompatible_type"
  )
})
...
test_that("names are never placed on the output", {
  ...
  expect_null(
    names(slide_period_vec(x, new_date(c(1, 2, 3, 4, 5)), "day", ~.x))
  )
  expect_null(
    names(
      slide_period_vec(
  ...
  expect_null(
    names(slide_period_int(x, new_date(c(1, 2, 3, 4, 5)), "day", ~.x))
  )
})
