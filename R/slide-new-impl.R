slide_new_impl <- function(.x,
  ...
  iterations <- iterations2(startpoint, endpoint, .before, .after, .step, .offset, .complete, forward)
...
iterations2 <- function(startpoint, endpoint, before, after, step, offset, complete, forward) {
  if (forward) {
  ...
  n_iter_frame_pos <- compute_n_iter(endpoint, frame_pos, step, forward)
  n_iter_frame_boundary <- compute_n_iter(endpoint, frame_boundary, step, forward)

  min(n_iter_frame_pos, n_iter_frame_boundary)
}

compute_n_iter <- function(endpoint, loc, step, forward) {
  diff <- endpoint - loc
