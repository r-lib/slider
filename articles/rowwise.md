...
[`slide()`](https://slider.r-lib.org/reference/slide.md) a *row-wise
...
[`slide()`](https://slider.r-lib.org/reference/slide.md) as a row wise
iterator in general. We’ll start with this simple data frame.
...
[`slide()`](https://slider.r-lib.org/reference/slide.md), it would
...
[`slide()`](https://slider.r-lib.org/reference/slide.md), on the other
hand, iterates over rows. This is consistent with the vctrs idea of
*size*, which is the length of an atomic vector, but the number of rows
of a data frame or matrix.
[`slide()`](https://slider.r-lib.org/reference/slide.md) always returns
an object with the same *size* as its input. Because the number of rows
in `example` is 4, the output size is 4 and you get one row per element
in the output.
...
[`slide()`](https://slider.r-lib.org/reference/slide.md) to control the
window size.
...
Often, using [`slide()`](https://slider.r-lib.org/reference/slide.md)
with its defaults will be enough, as it is common to iterate over just
one row at a time.
...
With [`slide()`](https://slider.r-lib.org/reference/slide.md) you can
pass these parameters on to
...
When [`slide()`](https://slider.r-lib.org/reference/slide.md)-ing inside
of a [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html)
call, there are a few scenarios that can arise. First, you might want to
slide over a single column. This is easy enough in both the un-grouped
and grouped case.
...
[`slide()`](https://slider.r-lib.org/reference/slide.md) iterates
