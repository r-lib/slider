...
- [`slide()`](https://slider.r-lib.org/reference/slide.md) iterates over
  your data like
...
- [`slide_index()`](https://slider.r-lib.org/reference/slide_index.md)
  computes a rolling calculation *relative to an index*. If you have
  ever wanted to compute something like a “3 month rolling average”
  where the number of days in each month is irregular, you might like
  this function.

- [`slide_period()`](https://slider.r-lib.org/reference/slide_period.md)
  is similar to
  [`slide_index()`](https://slider.r-lib.org/reference/slide_index.md)
  in that it slides relative to an index, but it first breaks the index
  up into “time blocks”, like 2 month blocks of time, and then it slides
  over `.x` using indices defined by those blocks.

Each of these core functions have the same variants as
[`purrr::map()`](https://purrr.tidyverse.org/reference/map.html). For
example, [`slide()`](https://slider.r-lib.org/reference/slide.md) has
[`slide_dbl()`](https://slider.r-lib.org/reference/slide.md),
[`slide2()`](https://slider.r-lib.org/reference/slide2.md), and
[`pslide()`](https://slider.r-lib.org/reference/slide2.md), along with
the other combinations of these variants that you might expect from
...
[`slide_dbl()`](https://slider.r-lib.org/reference/slide.md) for the
most common use cases. These include
[`slide_sum()`](https://slider.r-lib.org/reference/summary-slide.md) for
rolling sums and
...
[`slide()`](https://slider.r-lib.org/reference/slide.md) handles this
with a combination of the `.before` and `.after` arguments, which
...
Unlike [`purrr::map()`](https://purrr.tidyverse.org/reference/map.html),
[`slide()`](https://slider.r-lib.org/reference/slide.md) iterates over
data frames in a row wise fashion. Interestingly this means the default
of [`slide()`](https://slider.r-lib.org/reference/slide.md) becomes a
...
window that [`slide()`](https://slider.r-lib.org/reference/slide.md)
provides. You can use
[`slide_index()`](https://slider.r-lib.org/reference/slide_index.md) to
pass in both `.x` and an index, `.i`, and the window will be calculated
relative to that index.

Here, when computing a “2 day window”, you probably don’t want
`"2019-08-16"` and `"2019-08-18"` to be grouped together.
[`slide()`](https://slider.r-lib.org/reference/slide.md) has no concept
of an index, so when you specify a window size of 2, it will group these
two together.
[`slide_index()`](https://slider.r-lib.org/reference/slide_index.md), on
the other hand, will do the right thing.
...
`weeks()` or [`months()`](https://rdrr.io/r/base/weekday.POSIXt.html)):
...
[`slide_period()`](https://slider.r-lib.org/reference/slide_period.md)
is different from
[`slide_index()`](https://slider.r-lib.org/reference/slide_index.md) in
that it first breaks the index into “time blocks” and then slides over
`.x` relative to those blocks. For example, in the monthly period slide
below, `i` is broken up into 4 time blocks of “the current block of
monthly data, plus one block before this one”. The locations of those
blocks are the locations that are used to slice `.x` with.
...
[`slide_period()`](https://slider.r-lib.org/reference/slide_period.md)
is aware of the distance between elements of `.i` in the period you
specify. The practical implication of this is that in the above example,
group 3 with `2019-04-01` did *not* include `2019-02-05` in it, because
it is more than 1 month group away.
...
- With [`slide_vec()`](https://slider.r-lib.org/reference/slide.md) you
  can return any kind of object, and are not limited to the suffixed
  versions: `_dbl`, `_int`, etc.
...
best to deprecate `tsibble::slide()` in favor of
[`slider::slide()`](https://slider.r-lib.org/reference/slide.md).
...
that
[`slide_index()`](https://slider.r-lib.org/reference/slide_index.md)
tries to solve. Their solution is robust and quite fast, and has been a
nice benchmark for slider. slider is trying to solve a much narrower
problem, so the API here is more focused.

## Performance

Like [`purrr::map()`](https://purrr.tidyverse.org/reference/map.html),
the core functions of slider, such as
[`slide()`](https://slider.r-lib.org/reference/slide.md) and
[`slide_index()`](https://slider.r-lib.org/reference/slide_index.md),
are optimized in C to be as fast as possible, but there is overhead
involved in calling `.f` repeatedly. These functions are meant to be as
*general purpose* as possible, at the cost of some performance. This
means that slider can be used for more abstract computations, like
...
or
[`slide_index_sum()`](https://slider.r-lib.org/reference/summary-index.md).
These compute their corresponding metric at the C level, using a
specialized algorithm, and are often much faster than their
